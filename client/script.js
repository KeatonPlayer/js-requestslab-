...
// CODE HERE 
 const hello = document.querySelector('#say-hello-button')
...
hello.addEventListener('mouseover',sayHelloButton)
...
// CODE HERE
function takeButtonOff(){
    hello.style.backgroundColor = ''
    hello.style.color = ''
}

hello.addEventListener('mouseout',takeButtonOff)
...
axios.get('http://localhost:3000/animals').then((res) =>{
    ...
    let bod = document.querySelector('body')
     ...
     pcre.textContent=res.data[i]
     bod.appendChild(pcre)
}
})
}
...
const repeatMyParam = () => {
axios.get('http://localhost:3000/repeat/cat').then(function(res){
...
// CODE HERE
function getARequest(){
axios.get('http://localhost:3000/query-test/?coolname=bobmgee').then(function(res){

console.log(res.data)
})
}

document.getElementById('query-button').addEventListener('click',getARequest)
